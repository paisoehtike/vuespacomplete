<template>
  <div class="home-customer-row">
    ...
    >
      ...
      <OrderStepChip
        v-if="request.complete_at_by_5BB != null"
        :status="'Finish'"
        :value="'Finish'"
        slot="order-chip"
      ></OrderStepChip>
      <OrderStepChip
        v-else-if="request.complete_at_by_lsp != null && request.complete_at_by_5BB == null"
        :status="'Complete'"
        :value="'Complete'"
        slot="order-chip"
      ></OrderStepChip>
      <OrderStepChip
        v-else-if="request.installation_step != null && request.complete_at_by_5BB == null"
      ...
      <CustomerIssueDate v-if="request.created_at != null" slot="customer-date">
        ...
        <template
          v-slot:issue
          v-if="request.estimated_issue != null"
        >{{ request.estimated_issue.name }}</template>
      </CustomerIssueDate>
      ...
      <CustomerHomeFooterButton v-show="type !== 'team'" slot="customer-home-footer">
        ...
        <template
          v-else-if="request.lsp_team && isHistory == false && request.complete_at_by_lsp == null && request.complete_at_by_5BB == null && request.complete_at_by_lsp_team == null"
          v-slot:isAccept
        >
...
export default {
  ...
  data() {
    return {
      ...
      apis: {
        ...
        new:
          "https://sdm.5bb.com.mm/api/api/installation_requests?type=new",
        accepted:
          "https://sdm.5bb.com.mm/api/api/installation_requests?type=accepted",
        history:
          "https://sdm.5bb.com.mm/api/api/installation_requests?type=history",
        oncallNew:
          "https://sdm.5bb.com.mm/api/api/on_call_requests?type=new",
        oncallAccepted:
          "https://sdm.5bb.com.mm/api/api/on_call_requests?type=accepted",
        oncallHistory:
          "https://sdm.5bb.com.mm/api/api/on_call_requests?type=history",
        lspTeamRemain:
          "https://sdm.5bb.com.mm/api/api/lsp_team/home?type=remaining",
        lspTeamHistory:
          "https://sdm.5bb.com.mm/api/api/lsp_team/home?type=history",
        lspTeamComplete:
          "https://sdm.5bb.com.mm/api/api/lsp_team/home?type=complete"
      }
    };
  },
  beforeMount() {
  ...
  methods: {
    ...
    toOrder(request, event) {
      if (event.target.id == "accept") {
        if (this.request_type == "on_call") {
          axios
            .post(this.base_url + "on_call_requests_accepted/" + request.id)
            .then(response => {
          ...
          axios
            .post(
              this.base_url + "installation_requests_accepted/" + request.id
            )
            .then(response => {
              if (response.data.code == 200) alert("Successfully Professed!");
              this.filterRequest(request.id);
        ...
        this.$router.push({
          name: "order",
          params: { id: request.id, orderType: this.request_type }
        });
        // if(this.request_type == 'on_call') {
    ...
    toTeamOrder(request) {
      this.$router.push({
        name: "lsp-order",
        params: { id: request.id, orderType: request.request_type }
      });
    },
    getTeams() {
      if (
        (this.type == "admin" && this.status == "accepted") ||
        this.status == "oncall-accepted"
      ) {
        axios
          .get(this.base_url + "team_lists")
          .then(res => {
            this.bindTeams(res);
          })
